...
- Suite test: 79/79 PASSED (`pytest tests/`)
- Prossimo task tecnico: M6a/T43 (endpoint `GET /` + `templates/index.html`). T41+T42 CHIUSI. M5/T40 resta IN SOSPESO (vedi In sospeso), procede per scelta utente
...
| M6a/M6b FastAPI+UI+suggest+errori | M6a IN CORSO: T41+T42 CHIUSI (codice+runtime) | T41 `app/main.py`+`app/config.py`; T42 `launcher.py`; T43-T44/T46-T47 da fare |
...
- T42 CHIUSO 2026-05-16 (`launcher.py` in-process uvicorn + port fallback 8000->8003 + auto-browser; suite 79/79; smoke reale: 8000 free->8000, 8000 busy->8001, HTTP 200). Commit IN ATTESA di ok utente.
- T40 resta in sospeso ma il breakdown procede.
- Prossimo task: M6a/T43 (endpoint `GET /` -> `templates/index.html`).

## Riferimenti documentali

- `PROMPT_LOG.md`: registro iterazioni (ultimo: PLOG-2026-05-16-044).
